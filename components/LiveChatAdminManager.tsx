...
export default function LiveChatAdminManager({ isRtl }: LiveChatAdminManagerProps) {
  ...
  return (
    <div
      className={`card p-0 bg-white border border-primary-200 shadow-sm rounded-2xl overflow-hidden flex flex-col md:flex-row h-[720px] max-h-[calc(100vh-180px)] ${
    ...
    >
      ...
      <div
        className={`w-full md:w-80 lg:w-[340px] ${
          isRtl ? "border-b md:border-b-0 md:border-l" : "border-b md:border-b-0 md:border-r"
        } border-primary-200 bg-primary-50/50 flex flex-col shrink-0 h-full overflow-hidden`}
      >
        {/* Header */}
        <div className="p-3.5 border-b border-primary-200 bg-white space-y-2.5 shrink-0">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-navy-900 flex items-center gap-2 text-sm">
              ...
              <span>{isRtl ? "שיחות צ'אט חי" : "Live Chat Sessions"}</span>
            </h3>
          ...
          <div className="relative">
            <Search className={`w-3.5 h-3.5 absolute ${isRtl ? "right-3" : "left-3"} top-2.5 text-primary-400`} />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder={isRtl ? "חפש לפי מזהה, עיר, דף, טקסט..." : "Search by ID, city, page, text..."}
              className={`w-full ${isRtl ? "pr-9 pl-3 text-right" : "pl-9 pr-3 text-left"} py-1.5 bg-primary-50 border border-primary-200 rounded-xl text-xs focus:outline-none focus:border-gold-400 text-navy-900 font-medium`}
            />
          </div>
            ...
            filteredSessions.map((session) => {
              ...
              return (
                <button
                  ...
                  className={`w-full p-3 ${isRtl ? "text-right" : "text-left"} transition duration-150 flex flex-col gap-1.5 ${
                    isSelected
                      ? `bg-white ${isRtl ? "border-r-4 border-r-gold-500" : "border-l-4 border-l-gold-500"} shadow-sm`
                      : "hover:bg-white/80"
                  }`}
                >
                  <div className="flex items-center justify-between">
      ...
      <div className="flex-1 flex flex-col bg-white h-full min-w-0 overflow-hidden">
        {selectedSession ? (
          <>
            {/* Top Bar Header */}
            <div className="p-3.5 border-b border-primary-200 bg-white flex flex-col gap-2 shadow-sm shrink-0">
              <div className="flex items-center justify-between">
            ...
            {/* Message Stream */}
            <div className="flex-1 overflow-y-auto p-4 space-y-3.5 bg-primary-50/40 min-h-0">
            ...
            <div className="px-3.5 py-2 bg-primary-100/60 border-t border-primary-200/60 flex items-center gap-1.5 overflow-x-auto no-scrollbar shrink-0">
            ...
            {/* Admin Input Bar */}
            <div className="p-3.5 border-t border-primary-200 bg-white space-y-2 shrink-0">
